...
def render_vpn_toml(srv: ET.Element, *, include_credentials: bool = True) -> str:
...
def render_hosts_toml(srv: ET.Element, *, hostname: str) -> str:
    ...
    allowed_sni = _node_csv(srv, "allowed_sni")
    normalized_hostname = hostname.rstrip(".").lower()
    for alias in allowed_sni:
        normalized_alias = alias.rstrip(".").lower()
        labels = normalized_alias.split(".")
        if (
            len(normalized_alias) > 253
            or len(labels) < 2
            or any(
                not label
                or len(label) > 63
            ...
            raise ValueError(f"invalid allowed_sni hostname: {alias!r}")
        if normalized_alias.endswith("." + normalized_hostname):
            raise ValueError(
                f"allowed_sni {alias!r} conflicts with SNI authentication "
                f"syntax <credentials>.{hostname}"
            )
    ...
    lines: list[str] = [
        ...
        "allowed_sni = ["
        + ", ".join(_toml_escape(alias) for alias in allowed_sni)
        + "]",
    ]
    return "\n".join(lines) + "\n"
...
def main() -> int:
    ...
    try:
        ...
        rendered_files = [
            (
                OUT_DIR / "vpn.toml",
                render_vpn_toml(srv, include_credentials=bool(users)),
                0o644,
            ),
            (OUT_DIR / "hosts.toml", render_hosts_toml(srv, hostname=hostname), 0o644),
            (OUT_DIR / "credentials.toml", render_credentials_toml(users), 0o600),
            (OUT_DIR / "rules.toml", render_rules_toml(), 0o644),
        ]
        for path, content, mode in rendered_files:
